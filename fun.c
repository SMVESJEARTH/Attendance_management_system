...
void Admin_And_Ui(Employees* emp)//管理员功能
{
	...
	while (1)
	{
		...
		switch (pick)
		{
			...
			case 6: Range_Seek(); break;
			case 7: Information_Management(emp); break;
			case 8: ChangePassword(emp); break;
			case 0:flag = 1; printf("退出登录成功\n"); Sleep(commmon_time); system("cls"); Save(); break;
		}
		if (flag == 1)
			break;
...
void Range_Seek(void)
{
	...
	temp = scanf("%d", &month);
	if (month >= 10)
		sprintf(date, "%d%d", year, month);
	else
		sprintf(date, "%d0%d", year, month);

	sprintf(name, "%s%s%s", path, date,".txt");
	...
	while (fscanf(fp, "%d %d %d",&tn_be_l, &tn_c, &tn_a_v) == 3)
	{
		n_be_l += tn_be_l;
		n_c += tn_c;
		n_a_v += tn_a_v;
